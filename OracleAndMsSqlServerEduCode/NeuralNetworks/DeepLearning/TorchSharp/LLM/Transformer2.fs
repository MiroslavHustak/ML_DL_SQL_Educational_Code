...
module Transformer_TorchSharp2 =  //variant 2
    ...
    let private trainEpoch (model: torch.nn.Module<torch.Tensor, torch.Tensor>) (optimizer: torch.optim.Optimizer)
        ...
        |> List.iter 
            (fun epoch 
                ...
                use loss = lossFn.forward(output.view(-1L, int64 Settings.vocabSize), target.view(-1L))
    ...
    let internal main () =
        ...
        use scope = torch.NewDisposeScope()  //did not help
        ...
        trainEpoch model optimizer lossFn input target Settings.epochs "Pre-training"
        ...
        use fineTuneOptimizer = torch.optim.Adam(model.parameters(), lr = Settings.learningRate)
        
        model.train()

        trainEpoch model fineTuneOptimizer lossFn fineTuneInput fineTuneTarget Settings.fineTuneEpochs "Fine-tuning"
        ...
        generated |> List.iter (fun id -> printf "%s " (Settings.vocabulary |> List.item (int id)))        
        printfn "\n"

        model.Dispose()

        torch.CurrentDisposeScope.DisposeEverything()   //did not help
